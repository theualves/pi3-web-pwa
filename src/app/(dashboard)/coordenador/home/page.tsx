  ...
  useEffect(() => {
    ...
    if (dadosUsuario) {
      try {
        ...
        if (user && user.nome) {
          setNomeCoordenador(user.nome.split(" ")[0]);
        }
      } catch (e) {
    ...
    const carregarDados = async () => {
      ...
      try {
        ...
        const atividades = Array.isArray(dados)
          ? dados
          : dados.atividades || [];
        ...
        const rejeitadas = atividades.filter(
        ...
        const horasTotal = aprovadas.reduce(
          (acc: number, a: any) => acc + (a.horasAprovadas || 0),
          0,
        );
        const processadas = aprovadas.length + rejeitadas.length;
        const total = atividades.length;

        const hEnsino = aprovadas
          .filter((a: any) => a.categoria === "ENSINO")
          .reduce((acc: number, a: any) => acc + (a.horasAprovadas || 0), 0);
        const hPesquisa = aprovadas
          .filter((a: any) => a.categoria === "PESQUISA")
          .reduce((acc: number, a: any) => acc + (a.horasAprovadas || 0), 0);
        const hExtensao = aprovadas
          .filter((a: any) => a.categoria === "EXTENSAO")
          .reduce((acc: number, a: any) => acc + (a.horasAprovadas || 0), 0);
        ...
        setMetricas({
          ...
          taxaResolucao:
            total > 0 ? Math.round((processadas / total) * 100) : 0,
          taxaAprovacao:
            processadas > 0
              ? Math.round((aprovadas.length / processadas) * 100)
              : 0,
          taxaRejeicao:
            processadas > 0
              ? Math.round((rejeitadas.length / processadas) * 100)
              : 0,
          horasEnsino: hEnsino,
          horasPesquisa: hPesquisa,
          horasExtensao: hExtensao,
          totalHorasValidadas: horasTotal || 1,
        });
      } catch (error) {
        console.error("Erro ao montar o painel operacional:", error);
      } finally {
        setCarregando(false);
      }
    };
  ...
  if (carregando) {
    return (
      <div className="flex flex-col items-center justify-center h-[50vh] text-[#004A8D]">
        ...
        <p className="font-medium text-lg">Sincronizando fila de trabalho...</p>
      </div>
    );
  }

  return (
    <div className="flex flex-col gap-6 w-full overflow-x-hidden pb-10">
      <div>
        <h1 className="text-2xl font-bold text-slate-800">
          Bem-vindo(a), {nomeCoordenador || "Coordenador(a)"}
        </h1>
      </div>
          ...
          <div className="flex-1 flex flex-col justify-center gap-6">
            <div>
              <div className="flex justify-between items-center mb-2">
                ...
                <span className="text-sm font-bold text-[#004A8D]">
                  {metricas.horasEnsino}h
                </span>
              </div>
              <div className="w-full bg-slate-100 h-3 rounded-full overflow-hidden">
                <div
                  className="bg-[#004A8D] h-full rounded-full"
                  style={{
                    width: `${(metricas.horasEnsino / metricas.totalHorasValidadas) * 100}%`,
                  }}
                ></div>
              </div>
            </div>
            ...
            <div>
              <div className="flex justify-between items-center mb-2">
                ...
                <span className="text-sm font-bold text-[#DA761F]">
                  {metricas.horasPesquisa}h
                </span>
              </div>
              <div className="w-full bg-slate-100 h-3 rounded-full overflow-hidden">
                <div
                  className="bg-[#DA761F] h-full rounded-full"
                  style={{
                    width: `${(metricas.horasPesquisa / metricas.totalHorasValidadas) * 100}%`,
                  }}
                ></div>
              </div>
            </div>
            ...
            <div>
              <div className="flex justify-between items-center mb-2">
                ...
                <span className="text-sm font-bold text-[#210367]">
                  {metricas.horasExtensao}h
                </span>
              </div>
              <div className="w-full bg-slate-100 h-3 rounded-full overflow-hidden">
                <div
                  className="bg-[#210367] h-full rounded-full"
                  style={{
                    width: `${(metricas.horasExtensao / metricas.totalHorasValidadas) * 100}%`,
                  }}
                ></div>
              </div>
            </div>
